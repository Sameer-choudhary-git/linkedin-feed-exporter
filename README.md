...
The exporter writes a manifest and separate datasets under `data/linkedin-feed/latest` by default. The directory is ignored by Git because it may contain personal or third-party data. Complete per-post evidence capture is enabled by default; use `--no-element-capture` only when you want normalized records without raw card evidence.
