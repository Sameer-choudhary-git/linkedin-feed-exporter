...
export function isPostUrl(value = "") {
  ...
  try {
    ...
    return (
      /^\/feed\/update\/urn:li:(?:activity|share|ugcPost):\d+$/i.test(pathname) ||
...
export function parsePostId(url = "") {
  ...
  const urnMatch = pathname.match(/\/feed\/update\/(urn:li:(?:activity|share|ugcPost):\d+)/i);
...
export function classifyPost(snapshot) {
  ...
  if (snapshot.isPromoted || /\bPromoted\b/i.test(text)) return snapshot.job?.url ? "job" : "promoted";
  if (snapshot.job?.url || snapshot.hasJobCta || /\bview job\b/i.test(text)) return "job";
...
export function extractJob(snapshot) {
  ...
  const isPromoted = Boolean(snapshot.isPromoted || /\bPromoted\b/i.test(text));
  const hasJobSignal = Boolean(jobUrl || snapshot.hasJobCta || /\bview job\b/i.test(text));
  if (!hasJobSignal || (isPromoted && !jobUrl)) return null;
